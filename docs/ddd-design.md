...
| `SitePublisher` | HTML + feed.xml の書き出し | StaticSitePublisher（P2）。公開は Actions の deploy-pages |
...
3. reports = site の既存レポートを走査し直近 N 件
4. feed = FeedAssembler.assemble(reports)
   SitePublisher.write_feed(feed)                      # feed.xml + index.html
5. （Actions が site/ を Pages へデプロイ）
6. GenerationRun を集計してログ出力
```
...
| StaticSitePublisher | 文字列テンプレで HTML、`xml.etree` 風の手組みで RSS 2.0 |
...
├ src/
│  ├ domain/        keyword.py, report.py, slug.py, content_policy.py, feed.py
...
│  ├ config.py      # 環境変数の読み取り
│  └ main.py        # 配線・エントリポイント
├ site/             # Pages 公開対象（生成物。.gitignore 済み）
...
- **Phase 3 冗長化**: Anthropic フォールバック + 複数キーワード + 直近 N 件。検証=OpenAI 無効でも Anthropic 生成、複数 Issue 全件レポート化。
